// script.js — PhotoFix final
// Features: templates, signature 160×60, signature size 31–49 KB, auto-trim, preview, download

// ---------- TEMPLATES ----------
const TEMPLATES = {
  "rrb-ntpc": {
    id:"rrb-ntpc",
    title:"RRB NTPC — Photo",
    width:413, height:531,
    kb:50,
    format:"image/jpeg",
    description:"RRB NTPC recommended: 413×531 px."
  },
  "ssc-gd": {
    id:"ssc-gd",
    title:"SSC GD — Photo",
    ...
    kb:50,
    format:"image/jpeg",
    description:"SSC GD recommended: 300×400 px."
  },
  "signature": {
    id:"signature",
    title:"Signature",
    width:160, height:60,
    kb:40,                    // visible value; actual enforced 31–49 KB
    format:"image/jpeg",
    description:"Signature (RRB NTPC) — 160×60 px — final size 31–49 KB."
  }
};
...
// scale-up helper with faint border + tiny noise for signature
async function enforceMinSizeByScaling(baseCanvas, minBytes, mime='image/jpeg', quality=0.95, isSignature=false){
  ...
  for(let s=1.1; s<=1.6; s+=0.1){
    ...
    if(isSignature){
      ...
      tctx.lineWidth = Math.max(1, Math.round(tmp.height * 0.01));
      tctx.strokeRect(1,1,tmp.width-2,tmp.height-2);
    }
  ...
  if(isSignature){
    ...
    nctx.fillStyle = '#fff';
    nctx.fillRect(0,0,tmp2.width,tmp2.height);
    nctx.drawImage(baseCanvas,0,0,tmp2.width,tmp2.height);
    try{
      const id = nctx.getImageData(0,0,tmp2.width,tmp2.height);
      const pdata = id.data;
      const totalPixels = tmp2.width * tmp2.height;
      const noiseCount = Math.max(20, Math.round(totalPixels * 0.004));
      for(let i=0;i<noiseCount;i++){
        const px = Math.floor(Math.random()*tmp2.width);
        const py = Math.floor(Math.random()*tmp2.height);
        const idx = (py*tmp2.width + px)*4;
        pdata[idx]   = Math.max(0, pdata[idx]   - Math.floor(Math.random()*30));
        pdata[idx+1] = Math.max(0, pdata[idx+1] - Math.floor(Math.random()*30));
        pdata[idx+2] = Math.max(0, pdata[idx+2] - Math.floor(Math.random()*30));
        pdata[idx+3] = 255;
      }
      nctx.putImageData(id,0,0);
    }catch(e){ console.warn('Noise add failed',e); }
    const blob2 = await new Promise(res => tmp2.toBlob(b => res(b), mime, quality));
    return blob2;
  }

  const finalCanvas = document.createElement('canvas');
  finalCanvas.width = Math.max(1, Math.round(baseCanvas.width * 1.6));
  finalCanvas.height = Math.max(1, Math.round(baseCanvas.height * 1.6));
  finalCanvas.getContext('2d').drawImage(baseCanvas,0,0,finalCanvas.width,finalCanvas.height);
...
async function resizeImage(file, targetW, targetH, kbLimit, format, autoTrim=false){
  ...
  let minBytes = Math.max(512, Math.round(kbLimit*1024*0.5));
  if(autoTrim){            // signature
    minBytes = 31*1024;
    maxBytes = 49*1024;
  }
  ...
  if(format === 'image/png'){
    ...
    if(blob.size < minBytes){
      blob = await enforceMinSizeByScaling(finalCanvas, minBytes, 'image/png', 1.0, autoTrim);
    }
    return blob;
  } else {
    let quality = 0.95;
    ...
    if(blob.size < minBytes){
      const bigger = await enforceMinSizeByScaling(finalCanvas, minBytes, 'image/jpeg', 0.95, autoTrim);
...
processBtn.addEventListener('click', async () => {
  ...
  if(!w || !h){
    alert('Width / Height sahi bharein.');
